...
<div class="container bg-info">
  ...
  <form action="" method="POST">
    ...
    <div class="form-group">
      <label for="harga">Harga Lapangan Per Jam : </label>
      <input type="text" class="form-control" id="harga" name="harga" readonly value="<?php echo $r['harga_lapangan']?>">
    </div>
    <div class="form-group">
      <label for="poin">Bonus Poin Per Jam : </label>
      <input type="text" class="form-control" id="poin" name="poin" readonly value="<?php echo $r['poin_lapangan']?>">
    </div>
    <div class="form-group">
      ...
      <script>
          function totalHarga(waktu){
            a = document.getElementById("harga").value;
            c = document.getElementById("poin").value;
            totalh = a * waktu;
            totalh2 = c * waktu;
            document.getElementById("total").value = totalh;
            document.getElementById("poin1").value = totalh2;


            jamUser = +document.getElementById("jam").value;
            durasimain = +document.getElementById("durasi").value;
    ...
    <div class="form-group">
      <label for="poin1">Total Bonus Poin : </label>
      <input type="text" class="form-control" id="poin1" name="poin1" readonly value="<?php echo $r['poin_lapangan']?>">
    </div>
  ...
  if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if ($smile > 1){
      ...
      if ($sql = mysqli_query($koneksi,"INSERT INTO pesanan(nama_user, id_user, id_lapangan, tgl_pesanan, tgl_main, jam_mulai, jam_berakhir, total_harga, total_poin, status_pesanan) VALUES ('$nama', '$id', '$id_lapangan',LOCALTIME(),'$tgl_main',$jam_main,$jam_berakhir,$total_harga,$total_poin,'pending')")){      
        $_SESSION["tgl_main"] = $tgl_main;
        $_SESSION["jam_main"] = $jam_main;
        $sql = mysqli_query($koneksi,"SELECT * FROM pesanan WHERE nama_user = '$nama' AND id_user = '$id' AND id_lapangan = '$id_lapangan' AND tgl_main = '$tgl_main' AND jam_mulai = $jam_main AND  jam_berakhir = $jam_berakhir AND total_harga = $total_harga AND total_poin = $total_poin");
        $r = mysqli_fetch_array( $sql );
        ...
        <script type='text/javascript'> document.location = 'pembayaran.php'; </script>
      <?php
      }else {
          echo "error";
      }
